...
let conversation; let facilities = []; let profile = JSON.parse(localStorage.getItem('medinavProfile') || '{"language":"English","medications":[]}');
...
function addMessage(text, user = false) { const el = document.createElement('div'); el.className = `msg${user ? ' user' : ''}`; el.innerHTML = `<div class="avatar">${user ? 'You' : 'M'}</div><div class="bubble">${escapeHtml(text)}<small>${user ? 'You' : 'MediNav'}</small></div>`; messages.append(el); messages.scrollTop = messages.scrollHeight; }
...
function updateFacilities(result) {
  ...
  [[facilities[0], 'fac1', 'fac1p', 'dist1', 'fac1Actions'], [facilities[1], 'fac2', 'fac2p', 'dist2', 'fac2Actions']].forEach(([facility, name, detail, distance, action]) => {
    if (!facility) return; $(`#${name}`).textContent = facility.name; $(`#${detail}`).textContent = `${facility.type}${facility.address ? ` · ${facility.address}` : ''}`; $(`#${distance}`).textContent = facility.distance || 'Nearby';
...
async function startConversation(firstMessage) {
  ...
  try {
    ...
    conversation = await Conversation.startSession({
      agentId, dynamicVariables: { language: selectedLanguage(), user_name: profile.name || 'Guest' },
      ...
      onMessage: ({ message, role }) => message && addMessage(message, role === 'user'), onAgentToolResponse: handleToolResponse,
...
async function sendMessage(message = input.value.trim()) { if (!message) return; if (conversation) conversation.sendUserMessage(message); else await startConversation(message); input.value = ''; }
